/* =========================================================
   CRM API – OdontoCRM  (n8n + Supabase)
   ---------------------------------------------------------
   - Configure o prefixo do seu n8n ANTES deste arquivo:
       <script>window.CRM_API_BASE='https://SEU-N8N.cloud/webhook/odonto';</script>
     (opcional) <script>window.CRM_API_KEY='...';</script>
   ========================================================= */

/** ============ CONFIG ============ **/
const API_BASE = (window.CRM_API_BASE || 'https://allnsnts.app.n8n.cloud/webhook/odonto'); // APENAS o prefixo
const baseHeaders = () => ({
...
const qs = (obj={})=>{
  ...
  Object.entries(obj).forEach(([k,v])=> (v!==undefined && v!==null && v!=='') && p.append(k,String(v)));
...
export async function buscaCEP(cepRaw){
...
export function attachCepAutofill(formEl){
...
async function loadProcedures(){
  ...
  if(selAg){
    ...
    selAg.onchange = ()=>{ const d=selAg.selectedOptions[0]?.dataset?.dur; if(d&&dur) dur.value=`${d} min`; };
  }
...
async function createProcedure({name,duration,price,code}){
  ...
  await loadProcedures(); return created;
}
...
/** ============ PACIENTES (Supabase via n8n) ============ **/
export async function upsertPaciente(payload){
  // POST /patient/upsert  (merge por ID; podemos trocar para cpf+org se preferir)
  return api('/patient/upsert',{method:'POST', body:payload});
}
async function listPatients(limit=50){  // fallback para carregar primeiros
  return api('/patients'+qs({limit}));
}
async function searchPatients(q){       // autocomplete
  return api('/patients/search'+qs({q}));
}
...
/** ============ AGENDAMENTOS (Google Calendar via n8n) ============ **/
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  Promise.all([ loadProcedures(), setupPatientsDatalist() ]).catch(console.error);
  ...
  if(formPac){
    attachCepAutofill(formPac); // CEP → endereço
    formPac.addEventListener('submit', async (e)=>{
      ...
      const payload={
        ...
        address:{
          ...
          street:  f.street?.value || null,
          number:  f.number?.value || null,
      ...
      try{
        ...
        // Atualiza datalist (carrega novamente os 50 primeiros)
        setupPatientsDatalist();
      }catch(err){ alert('Erro ao salvar paciente'); console.error(err); }
    });
  }
